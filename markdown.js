...
function erimd(content,isProfile) {
    ...
    const text = content
        .replace(/(?:^|\s|<p>)@([\w-]+)(?![^<]*?<\/code>)/g, (match, p1) => {
            const prefix = match.startsWith('<p>') ? '<p>' : '';
            return `${prefix}<span id="username" class="attachment" onclick="${fixProfile}openUsrModal(\'${p1}\')">@${p1}</span>`;
        })
...
function createButtonContainer(p) {
    ...
    <div class='toolbarContainer'>
        ...
        ${page !== "recent" ? `<div class='toolButton tooltip' onclick='pingusr(event)' aria-label="ping" data-tooltip="${lang().action.ping}" title="ping" tabindex="0">
